import React from 'react'
import { useNavigate } from 'react-router-dom'

const ErrorPage = () => {
  const navigate = useNavigate();

  return (
    <div style={{
      display: 'flex',
      flexDirection: 'column',
      alignItems: 'center',
      justifyContent: 'center',
      minHeight: '100vh',
      backgroundColor: 'var(--sx-bg)',
      color: 'var(--sx-text-primary)',
      fontFamily: 'var(--font-sans)',
      padding: '20px',
      textAlign: 'center'
    }}>
      <span style={{ fontSize: '72px', marginBottom: '16px' }} role="img" aria-label="magnifying glass">🔍</span>
      <h1 style={{ fontSize: '36px', fontWeight: '800', marginBottom: '12px', color: 'var(--sx-text-primary)' }}>Page Not Found (404)</h1>
      <p style={{ fontSize: '16px', color: 'var(--sx-text-muted)', maxWidth: '480px', marginBottom: '24px', lineHeight: '1.6' }}>
        We couldn't find the page you were looking for. It may have been moved, deleted, or never existed in the first place.
      </p>
      <button 
        onClick={() => navigate('/')}
        style={{
          padding: '12px 24px',
          backgroundColor: 'var(--sx-gold)',
          color: '#0F172A',
          border: 'none',
          borderRadius: '8px',
          fontWeight: '700',
          fontSize: '14px',
          cursor: 'pointer',
          transition: 'opacity 0.2s'
        }}
      >
        Return to Home Page →
      </button>
    </div>
  )
}

export default ErrorPage